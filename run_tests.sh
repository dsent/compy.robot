...
# fake_firmware.py on a pseudo-terminal, hands the device to
# the suite as arg[1], and kills the firmware when the suite
# touches /tmp/kill_now -- which is how the suites test a
# device that dies mid-session.
#
set -uo pipefail

cd "$(dirname "$0")"

PTY_SUITES=(test_robot_serial.lua test_robot_transport.lua
            test_robot_move.lua)
...
run_pty_suite() {
  ...
  setsid uv run python fake_firmware.py >/tmp/fake_fw.log 2>&1 &
  fw_pid=$!
...
if [[ $# -gt 0 ]]; then
  ...
  for suite in test_robot_line_reader.lua "${PTY_SUITES[@]}"; do
